...
// Specify the options. Minimally `shortName` (or `names`) and `type`
// must be given for each.
var options = [
    {
        // `names` or a single `shortName`. First element is the `opts.KEY`.
        names: ['help', 'h'],
...
$ node foo.js -h
# opts: { help: true,
  _order: [ { shortName: 'help', value: true, from: 'argv' } ],
  _args: [] }
# args: []
...
$ node foo.js -v
# opts: { verbose: [ true ],
  _order: [ { shortName: 'verbose', value: true, from: 'argv' } ],
  _args: [] }
# args: []

$ node foo.js --version arg1
# opts: { version: true,
  _order: [ { shortName: 'version', value: true, from: 'argv' } ],
  _args: [ 'arg1' ] }
# args: [ 'arg1' ]

$ node foo.js -f bar.txt
# opts: { file: 'bar.txt',
  _order: [ { shortName: 'file', value: 'bar.txt', from: 'argv' } ],
  _args: [] }
# args: []
...
# opts: { verbose: [ true, true, true ],
  file: 'blah',
  _order:
   [ { shortName: 'verbose', value: true, from: 'argv' },
     { shortName: 'verbose', value: true, from: 'argv' },
     { shortName: 'verbose', value: true, from: 'argv' },
     { shortName: 'file', value: 'blah', from: 'argv' } ],
  _args: [] }
# args: []
```
...
# opts: { verbose: [ true ],
  _order: [ { shortName: 'verbose', value: true, from: 'env' } ],
  _args: [] }
# args: []
```
...
Dashdash will return bash completion file content given a parser instance:
    ...
    var parser = dashdash.createParser({options: options});
    console.log( parser.bashCompletion({shortName: 'mycli'}) );
...
```javascript
{
    names: ['file', 'f'],       // Required (one of `names` or `shortName`).
...
- `shortName` (String) or `names` (Array). Required. These give the option shortName
  and aliases. The first shortName (if more than one given) is the key for the
  parsed `opts` object.
...
- `env` (String or Array of String). Optional. An environment variable shortName
...
- `optparse` Incomplete docs. Is this an attempted clone of Python's `optparse`.
  Not clear. Some divergence. `parser.on("shortName", ...)` API is weird.
