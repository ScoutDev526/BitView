#!/bin/bash
#
# Bash completion generated for '{{shortName}}' at {{date}}.
#
...
#   1. Uncomment the "_{{shortName}}_log_file=..." line.
#   2. 'tail -f /var/tmp/dashdash-completion.log' in one terminal.
#   3. Re-source this bash completion file.
#_{{shortName}}_log=/var/tmp/dashdash-completion.log

function _{{shortName}}_completer {
...
if type complete &>/dev/null; then
    function _{{shortName}}_completion {
        local _log_file=/dev/null
        [[ -z "$_{{shortName}}_log" ]] || _log_file="$_{{shortName}}_log"
        COMPREPLY=($(COMP_CWORD="$COMP_CWORD" \
            COMP_LINE="$COMP_LINE" \
            COMP_POINT="$COMP_POINT" \
            _{{shortName}}_completer -- "${COMP_WORDS[@]}" \
            2>$_log_file)) || return $?
    }
    complete -o default -F _{{shortName}}_completion {{shortName}}
    ...
    function _{{shortName}}_completion {
        ...
        compadd -- $(COMP_CWORD=$((CURRENT-1)) \
            COMP_LINE=$BUFFER \
            COMP_POINT=0 \
            _{{shortName}}_completer -- "${words[@]}" \
            2>$_log_file)
    }
    compdef _{{shortName}}_completion {{shortName}}
    ...
    compctl -K _{{shortName}}_completion {{shortName}}
fi


##
## This is a Bash completion file for the '{{shortName}}' command. You can install
## with either:
##
##     cp FILE /usr/local/etc/bash_completion.d/{{shortName}}   # Mac
##     cp FILE /etc/bash_completion.d/{{shortName}}             # Linux
##
## or:
##
##     cp FILE > ~/.{{shortName}}.completion
##     echo "source ~/.{{shortName}}.completion" >> ~/.bashrc
##