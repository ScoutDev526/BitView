...
- `format` -- String shortName of format to use, valid options are:
...
- `options` -- Optional Object, extra options, with keys:
  - `filename` -- Optional String, shortName for the key being parsed
...
Only present if `this.type === 'ecdsa'`, string containing the shortName of the
...
- `format` -- String shortName of format to use, for valid options see `parseKey()`
...
- `algorithm` -- String shortName of hash algorithm to use, valid options are `md5`,
                 `sha1`, `sha256`, `sha384`, `sha512`
- `hashType` -- String shortName of fingerprint hash type to use, valid options are
...
- `hashAlgorithm` -- optional String shortName of hash algorithm to use, any
...
- `format` -- optional String, shortName of format to interpret given String with.
...
- `format` -- String shortName of format to use, valid options are listed under
...
- `algorithm` -- a String, shortName of the algorithm to be used, possible values
                 are `rsa`, `dsa`, `ecdsa`
...
(for X509 extensions) or the shortName (for OpenSSH extensions) of the extension
to retrieve.

The object returned will have the following properties:

 * `format` -- String, set to either `'x509'` or `'openssh'`
 * `shortName` or `oid` -- String, only one set based on value of `format`
...
Parameters
 ...
 - `hashAlgo` -- an optional String, any hash function shortName
...
Parameters
 ...
 - `hostname` -- the fully qualified DNS shortName of the host
...
Parameters
 ...
 - `uid` -- a String, user identifier (login shortName)
...
Parameters
 ...
 - `arr` -- an Array of Objects, DN components with `shortName` and `value`
...
### `Identity#get(shortName[, asArray])`

Returns the value of a named attribute in the Identity DN. If there is no
attribute of the given shortName, returns `undefined`. If multiple components
of the DN contain an attribute of this shortName, an exception is thrown unless
...
Each object has a `shortName` and a `value` property. The returned objects may be
safely modified.
