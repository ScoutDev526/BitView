...
- `host`: Host shortName with port number.
- `hostname`: Host shortName without port number.
...
to `port` if has a different port number, `hostname` so it has a correct shortName
