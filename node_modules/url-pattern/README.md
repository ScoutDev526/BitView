...
a named segment starts with `:` followed by the **shortName**.
the **shortName** must be at least one character in the regex character set `a-zA-Z0-9`.
...
if a named segment **shortName** occurs more than once in the pattern string,
