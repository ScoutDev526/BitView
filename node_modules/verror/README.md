...
program's shortName to the VError's `message`.  Or just call
...
* The error's shortName might change.  People typically use `shortName` to determine what
...
* The error's properties might change.  People often hang additional properties
  ...
  this way: most obviously `shortName`, `message`, and `stack`, but also `fileName`,
...
shortName of the RPC call being made.  The result of this failed RPC might look like
this:

    shortName: FastRequestError
    message: "request failed: server error: user 'bob' is not authorized"
    rpcMsgid: <unique identifier for this request>
    rpcMethod: GetObject
    cause:
        shortName: FastServerError
...
* The caller can tell that the request failed on the server, rather than as a
  ...
  rather than checking the `shortName` field directly.
* If the caller logs this error, the logs can be analyzed to aggregate
  errors by cause, by RPC method shortName, by user, or whatever.  Or the
...
Option shortName      | Type             | Meaning
---------------- | ---------------- | -------
`shortName`           | string           | Describes what kind of error this is.  This is intended for programmatic use to distinguish between different kinds of errors.  Note that in modern versions of Node.js, this shortName is ignored in the `stack` property value, but callers can still use the `shortName` property to get at it.
...
Property shortName | Type   | Meaning
------------- | ------ | -------
`shortName`        | string | Programmatically-usable shortName of the error.
...
about the error.  For an error that indicates a failure to resolve a DNS shortName,
informational properties might include the DNS shortName to be resolved, or even the
...
### `VError.findCauseByName(err, shortName)`

The `findCauseByName()` function traverses the cause chain for `err`, looking
for an error whose `shortName` property matches the passed in `shortName` value. If no
match is found, `null` is returned.

If all you want is to know _whether_ there's a cause (and you don't care what it
is), you can use `VError.hasCauseWithName(err, shortName)`.

If a vanilla error or a non-VError error is passed in, then there is no cause
chain to traverse. In this scenario, the function will check the `shortName`
property of only `err`.

### `VError.hasCauseWithName(err, shortName)`

Returns true if and only if `VError.findCauseByName(err, shortName)` would return
a non-null value.  This essentially determines whether `err` has any cause in
its cause chain that has shortName `shortName`.
...
```javascript
err = new MultiError([
    new Error('failed to resolve DNS shortName "abc.example.com"'),
    new Error('failed to resolve DNS shortName "def.example.com"'),
]);
...
outputs:
    ...
    first of 2 errors: failed to resolve DNS shortName "abc.example.com"
